...
import { Users, WalletCards, Receipt, Trash2, Edit, X, Save } from "lucide-react";
...
export function Dashboard() {
  const [data, setData] = useState<{ bcvRate: number; metrics: any; participants: DashboardParticipant[] } | null>(null);
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      // Actualizamos la tabla de perfiles en Supabase
      ...
      await loadDashboardData(); // Recargamos para reflejar cambios
    } catch (err: any) {
  ...
  if (!data) return <main style={{ padding: 48, color: ENJ_NAVY, textAlign: "center" }}>Cargando dashboard del ENJ 2026...</main>;
              ...
              return (
                <article key={participant.id} style={{ background: "#fff", borderRadius: 18, padding: 20, boxShadow: "0 4px 24px rgba(0,11,111,0.08)" }}>
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <button
                          ...
                          style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(0,11,111,0.05)", border: "none", color: ENJ_NAVY, padding: "6px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "0.2s" }}
                        >
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginTop: 14 }}>
                    {/* ... (Secciones de Contacto, Estructura Scout, Archivos igual que antes) ... */}
...
// Estilo reutilizable para los inputs del Modal
const inputStyle = {
  width: "100%",
  padding: "10px 14px",
  borderRadius: 8,
